...
x = np.arange(-6, 6, 0.005)  # range of x values for plotter
aniObj = animationObj.animationObj(f, 0, x, timeString)  # initialize animation

# other buttons
button3 = Tk.Button(master=f1,
                    text='Make',
                    command=lambda: aniObj.addIt(int(options.get())),
                    height=2)
stopIt = Tk.Button(master=f1,
                   text='Stop',
                   command=aniObj.stopIt,
                   height=1)
startIt = Tk.Button(master=f1,
                    text='Start',
                    command=aniObj.startIt,
                    height=1)
...
# text entry box to recieve user input
e = Tk.Entry(f2)

button4 = Tk.Button(master=f2,
                    text='Add Initial Function',
                    command=lambda: dictionaries.addInitFunct(e, w, options),
                    height=1)
button5 = Tk.Button(master=f2,
                    text='Add Varying Function',
                    command=lambda: dictionaries.addVaryFunct(e),
                    height=1)

# final widget packings
